...
def check_rare_exon(INFOdict,index):# find variant with propability < 0.1%
   ...
   # if multiallele : 
    ...
    for key, value in INFOdict.iteritems():
	 ...
	 #   #print key+"  "+value+" "+str(index)
	 #   exit;
	#print key+':'+value+"\t"+str(index)+"\t"+str(len(tempstr));
 ...
 #   ExAcscore= max(map(float,[ rate  for rate in INFOdict.get('ExAC_ALL',str(threshold)).split(',') if rate != '.' ])+[0])
    ...
    if  MutationFunct in CodingCodes :
        return INFOstring
  #  if  KGscore <= threshold and ESPscore <= threshold and  MutationFunct in CodingCodes and ExAcscore <= threshold and laf<0.02 :
   #     return INFOstring
  #  else : 
 #       return ""
	...
	if int(INFOdict['AN']) < 2*(len(data)-9)*0.9 : continue
       ...
       #     print str(proband_index)+"\t"+pgt0
        ...
        #    print trio_GT[0] +"\t"+ str (cor) 
            if trio_GT != ['./.','./.','./.'] and trio_GT[0] !="0/0" and cor>0 :
             #   print '\t'.join(data[:3])
		INFOdict2=deepcopy(INFOdict);
		#print "here: "+INFOdict2['ExAC_ALL']
                infostr=check_rare_exon(INFOdict2,cor);
          #      print infostr
                if infostr :
                    wstr='\t'.join(data[:4])+'\t'+alt+'\t'+'\t'.join(data[5:7])+'\t'+infostr+'\t'+'\t'.join(data[8:9])+'\t'+'\t'.join(trio)+'\n'
