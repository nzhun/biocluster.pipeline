...
def variants_filter(filename, parameter):
    ...
    with open(filename,'Ur') as denovo_f:
        ...
        for line in denovo_r:
          ...
          #  print variant.get('SegDup')
            if variant.get('SegDup') in {'NA','.'} or re.match('.,',variant.get('SegDup')) :
                 segdup = 0
            else:
                if variant.get('SegDup',':0').split(':')[1].split(',')[0].split('-')[0].isdigit()==False:
                    segdup=1
                else:
                    segdup=float(variant.get('genomicSuperDups',':0').split(':')[1].split(',')[0].split('-')[0]) #
            ...
            for i in range(len(probands)) :
               ...
               # if real_AC > parameter ['snps']['max_AC']:
                ...
                varclass = variant['VarClass']
                if len(variant['VarClass'].split(',')) > 1:
                    varclass = variant['VarClass'].split(',')[alt_idx-1]
                    newline[idx['VarClass']] = varclass
