...
for line in f: 
    if line[0]=='#': # get and write head
        if line[1]=='#':
            ...
            f = [open(trio_dir_name+ ';'.join([proband, sample_index[proband][0], sample_index[proband][1] ]) + '.vcf', "w") for proband in sample_index]
            n = len(f)
            for i in range(n):
                proband = f[i].name.split('/')[-1].split(';')[0]
        ...
        for i in range(n):
            ...
            data[father_index].split(':')[0].replace('.','0'),\
            data[mother_index].split(':')[0].replace('.','0')]
            if trio_GT != ['./.','./.','./.'] :
