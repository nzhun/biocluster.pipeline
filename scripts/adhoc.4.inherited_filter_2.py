    ...
    with open(filename,'Ur') as denovo_f:
       ...
       # prefix='.'.join(filename.rsplit('.', 1)[:-1] )
        ...
        for line in denovo_r:
            ...
            if 'max_Multiallelic' in parameter['gene_filter']:
           ...
           # print segdup
            ...
            for i in range(len(probands)) :
                ...
                if alt_idx > len(alt.split(',')) : alt_idx=1 
                ...
                if len(variant['ExonicFunc.refGene'].split(',')) > 1:
                    newline[idx['ExonicFunc.refGene']] = variant['ExonicFunc.refGene'].split(',')[alt_idx-1]
