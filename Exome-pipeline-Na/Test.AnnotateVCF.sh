...
tail -n +2 $AnnFil | awk '{gsub( / /, \"\"); print}' | awk '{gsub( /;/, \",\"); print}' | awk '{gsub( /=/, \":\"); print}' | sort -k1,1V -k2,2n >> $AnnFil.tempheader; 
...
StepCmd="less $InpFil | vcf-annotate -a $AnnFil.byAF.gz
...
StepCmd="less $VcfFil | vcf-annotate -a $AnnFil.byfunc1.gz
...
> $VcfFilAnn.tmp"
funcRunStep
mv $VcfFilAnn.tmp $VcfFil


#Cut the function impact by other predictor/database
StepName="Make annotation table"
StepCmd="gunzip -c $AnnFil | cut -f 1-5,47-91 | head -n 1 > $AnnFil.byfunc2 ; 
gunzip -c $AnnFil | cut -f 1-5,47-91 | tail -n +2 >> $AnnFil.byfunc2 ; 
bgzip $AnnFil.byfunc2 ;
tabix -S 1 -s 1 -b 2 -e 2 $AnnFil.byfunc2.gz"
funcRunStep
...
#Incorporate annovar annotations into vcf with vcftools
StepName="Incorporate annovar annotations into vcf with vcftools"
StepCmd="less $VcfFil | vcf-annotate -a $AnnFil.byfunc2.gz
 ...
 -d key=INFO,ID=Kaviar_AN,Number=1,Type=Float,Description='Kaviar_AN' > $VcfFilAnn.tmp"
funcRunStep
mv $VcfFilAnn.tmp $VcfFil
...
gunzip -c $AnnFil | cut -f 1,2,3,4,5,6,7,65,72,92-96 | tail -n +2 | awk '!a[\$1\$2]++' >> $AnnFil.bylocus ; 
bgzip $AnnFil.bylocus ;
tabix -S 1 -s 1 -b 2 -e 2 $AnnFil.bylocus.gz"
funcRunStep

#Incorporate annovar annotations into vcf with vcftools
StepName="Incorporate annovar annotations into vcf with vcftools"
StepCmd="cat $VcfFil | vcf-annotate -a $AnnFil.bylocus.gz 
 -c CHROM,POS,-,REF,ALT,INFO/VarFunc,INFO/GeneName,INFO/SegDup,INFO/avsnp147
 ...
 -d key=INFO,ID=SegDup,Number=1,Type=String,Description='Genomic Segmental duplications, Score=fracMatchIndel from UCSC, Name=matching segment' 
 -d key=INFO,ID=avsnp147,Number=1,Type=String,Description='dbSNP147 with allelic splitting and left-normalization'> $VcfFilAnn.ann2;
