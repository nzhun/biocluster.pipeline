...
AlnFil=$BamNam.bwamem.cram #filename for bwa-mem aligned file
SngFil=$BamNam.singletons.gz #output file for the singletons to be dumped to
SrtFil=$BamNam.bwamem.sorted.cram #output file for sorted bam
DdpFil=$BamNam.bwamem.mkdup.cram #output file with PCR duplicates marked
...
if [[ ! $RGID ]]; then
    ...
    AlnFil=$BamNam.RGID_$RGID.bwamem.cram
    SngFil=$BamNam.RGID_$RGID.singletons.gz #output file for the singletons to be dumped to
    SrtFil=$BamNam.RGID_$RGID.bwamem.sorted.craam #output file for sorted bam
    DdpFil=$BamNam.RGID_$RGID.bwamem.mkdup.craam #output file with PCR duplicates marked
...
StepCmd=$StepCmd" gzip | bwa mem -M -R \"$RgHeader\" -t 8 -p $REF - |
 samtools view -CS -T $REF - > $AlnFil"
funcRunStep

#Sort the bam file by coordinate
StepName="Sort Bam using PICARD"
StepCmd="java -Xmx4G -XX:ParallelGCThreads=8 -Djava.io.tmpdir=$TmpDir -jar $PICARD SortSam
 INPUT=$AlnFil
 OUTPUT=$SrtFil
...
StepCmd="java -Xmx4G -XX:ParallelGCThreads=8 -Djava.io.tmpdir=$TmpDir -jar $PICARD MarkDuplicates
 INPUT=$SrtFil
 OUTPUT=$DdpFil
 REFERENCE_SEQUENCE=$REF
