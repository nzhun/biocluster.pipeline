...
def make_bed(bed_name, current_window):
    ...
    for item in current_window:
        fout.write(str(item.CHR) + '\t' + str(item.START) +
                   '\t' + str(item.END) + '\n')
    fout.close()
...
def make_script(bedfiles, samtools, bcftools, bamlist, ref_genome):
    ...
    for i, bed in enumerate(bedfiles):
        script.write('nohup $SAMTOOLS mpileup -u -t DP,AD,ADF,ADR,SP,INFO/AD,INFO/ADF,INFO/ADR -f $REFGENOME -l ' + bed + ' -b $BAMLIST 2>stderr_' +
                     str(i) + '.txt | nohup $BCFTOOLS call - -c -v 2>stderr_bcf_' + str(i) + '.txt > samtools.' + str(i) + '.vcf &\n\n')
