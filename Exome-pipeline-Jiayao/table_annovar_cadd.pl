...
our $DATE =	'$Date: 2013-11-12 18:10:55 -0800 (Tue, 12 Nov 2013) $';  
...
=head1
	for my $i (0 .. @header-1) {
		...
		} elsif ($header[$i] eq 'ljb23_all') {
			push @expanded_header, qw/LJB23_SIFT_score LJB23_SIFT_score_converted LJB23_SIFT_pred LJB23_Polyphen2_HDIV_score LJB23_Polyphen2_HDIV_pred LJB23_Polyphen2_HVAR_score LJB23_Polyphen2_HVAR_pred LJB23_LRT_score LJB23_LRT_score_converted LJB23_LRT_pred LJB23_MutationTaster_score LJB23_MutationTaster_score_converted LJB23_MutationTaster_pred LJB23_MutationAssessor_score LJB23_MutationAssessor_score_converted LJB23_MutationAssessor_pred LJB23_FATHMM_score LJB23_FATHMM_score_converted LJB23_FATHMM_pred LJB23_RadialSVM_score LJB23_RadialSVM_score_converted LJB23_RadialSVM_pred LJB23_LR_score LJB23_LR_pred LJB23_GERP++ LJB23_PhyloP LJB23_SiPhy/;
		} elsif ($header[$i] eq 'popfreq_all') {
			push @expanded_header, qw/PopFreqMax 1000G2012APR_ALL 1000G2012APR_AFR 1000G2012APR_AMR 1000G2012APR_ASN 1000G2012APR_EUR ESP6500si_ALL ESP6500si_AA ESP6500si_EA CG46/;
		} elsif ($header[$i] eq 'cadd') {
			push @expanded_header, qw/CADD_raw CADD_phred/;
		} elsif ($header[$i] eq 'caddgt10') {
			push @expanded_header, qw/CADD_raw CADD_phred/;
		} else {
...
=cut
	...
	my %annotation_headers = (
		...
		"caddgt10" => [ qw/CADD_raw CADD_phred/ ],
		"cadd" => [ qw/CADD_raw CADD_phred/ ]
		);
	for my $item (@header) {
...
=head1
			if ($item eq 'ljb_all') {
				...
				$expanded_field = 15;
			} elsif ($item eq 'ljb23_all') {
				$expanded_field = 27;
			} elsif ($item eq 'popfreq_all') {
				$expanded_field = 10;
			} elsif ($item eq 'cadd') {
				$expanded_field = 2;
			} elsif ($item eq 'caddgt10') {
				$expanded_field = 2;
			}
=cut
			if ( exists $annotation_headers{$item} ) {
...
sub filterOperation {
	...
	if ($dbtype =~ m/^ljb\d*_all/ or $dbtype =~ m/^ljb\d*_mod/ or $dbtype =~ m/^popfreq/ or $dbtype =~ m/^caddgt10/ or $dbtype =~ m/^cadd/) {
		$sc .= " -otherinfo";
...
=head1 SYNOPSIS
 ...
 Version: $Date: 2013-11-12 18:10:55 -0800 (Tue, 12 Nov 2013) $
