...
-a (required) - the start line of the array
-s (required) - the length of the array. ### e.g. -s 1000 -a 2; then the region between line 2000..3000 of the bed file  will process 
...
while getopts i:r:a:s:t:n:l:j:PXBH opt; do
    case "$opt" in
        i) InpFil="$OPTARG";;
        r) RefFil="$OPTARG";;
        a) ArrNum="$OPTARG";;
...
#TgtFil=$( head -n $((ArrNum*length+length)) $TgtBed)
TgtFil=$TmpDir/$ArrNum.$VcfNam.bed #exome capture range
head -n $((ArrNum+size)) $TgtBed | tail -n $size > $TgtFil #get exome capture range
if [ ! -s $TgtFil ]
then
exit;
fi
...
funcWriteStartLog
echo "Target file line range: $((ArrNum*size)) - $(( $ArrNum*size+size ))" >> $TmpLog

##Run Joint Variant Calling
StepName="Joint call gVCFs with GATK"
StepCmd="java -Xmx5G -XX:ParallelGCThreads=4 -Djava.io.tmpdir=$TmpDir -jar $GATKJAR
 -T GenotypeGVCFs
 -R $REF
 -L $TgtFil
